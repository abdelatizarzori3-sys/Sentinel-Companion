    ...
    public void play(PluginCall call) {
        ...
        getBridge().execute(() -> {
            try {
                ...
                int bufferSize = Math.max(minimumBuffer, bytes.length);
                ...
                int written = track.write(bytes, 0, bytes.length, AudioTrack.WRITE_BLOCKING);
                if (written != bytes.length) { stopTrack(); call.reject("AUDIO_BUFFER_WRITE_FAILED"); return; }
                track.play();
                ...
                playbackThread = new Thread(() -> {
                    try {
                        Thread.sleep(Math.max(800, (bytes.length * 1000L) / (sampleRate * 2)));
