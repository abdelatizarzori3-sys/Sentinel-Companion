...
const JokeLibrary = [
  { category: 'space', author: 'د. كوانتم — نص أصلي', ar: 'سول العالم راسو: علاش القمر ما كيدخلش للجامعة؟ جاوب: حيث ديجا عندو أطوار كثيرة.', darija: 'سول العالم راسو: علاش القمر ما كيدخلش للجامعة؟ جاوب: حيث ديجا عندو أطوار بزاف.', en: 'Why did the Moon skip university? It already had too many phases.' },
  { category: 'code', author: 'المهندس كومبايل — نص أصلي', ar: 'قال المبرمج للخطأ: أنت مجرد فكرة. أجابه الخطأ: صحيح، لكنني فكرة تتكرر عند كل تشغيل.', darija: 'قال المبرمج للخطأ: نتا غير فكرة. جاوبو الخطأ: واخا، ولكن فكرة كترجع مع كل تشغيل.', en: 'The programmer called the bug an idea. The bug replied: “True, but I recur on every run.”' },
  { category: 'physics', author: 'مختبر نيوتن المرح — نص أصلي', ar: 'لماذا كان الفوتون هادئًا؟ لأنه لا يحمل كتلة من الهموم، فقط طاقة.', darija: 'علاش الفوطون هاني؟ حيت ما حامل حتى كتلة ديال الهموم، غير الطاقة.', en: 'Why was the photon calm? It carried no mass of worries, only energy.' },
  { category: 'biology', author: 'الأستاذة خلية — نص أصلي', ar: 'قالت الخلية لجارتها: لا تقلقي، أنا منقسمة بين أمرين… لكن بطريقة علمية.', darija: 'قالت الخلية لجارتها: ما تقلقيش، أنا منقسمة بين جوج… ولكن بطريقة علمية.', en: 'The cell told its neighbor: “Don’t worry, I’m divided between two things—in a scientific way.”' }
];
let lastJokeIndex = -1;
function loadScientificJoke() { const category = $('joke-category')?.value || 'all'; const available = JokeLibrary.map((joke, index) => ({ joke, index })).filter(item => category === 'all' || item.joke.category === category); if (!available.length) return; let choice = available[Math.floor(Math.random() * available.length)]; if (available.length > 1 && choice.index === lastJokeIndex) choice = available.find(item => item.index !== lastJokeIndex) || choice; lastJokeIndex = choice.index; const text = Companion.locale === 'ar-MA' ? choice.joke.darija : Companion.locale === 'en-US' ? choice.joke.en : choice.joke.ar; $('joke-text').textContent = text; $('joke-author').textContent = `المؤلف: ${choice.joke.author}`; speakCompanion(text); }

const Companion = { messages: [], controller: null, voice: false, speaking: false, listening: false, sensors: false, locale: 'ar-MA', recognition: null, lastMotionAt: 0, weather: null, voices: [], voiceName: '', rate: 0.98, pitch: 1, lipTimer: null };

function findCompanionVoice() { return Companion.voices.find(voice => voice.name === Companion.voiceName) || Companion.voices.find(voice => voice.lang.toLowerCase().startsWith(Companion.locale.toLowerCase().split('-')[0])) || Companion.voices.find(voice => voice.lang.toLowerCase().startsWith('ar')) || Companion.voices[0]; }
function loadCompanionVoices() { if (!('speechSynthesis' in window)) return; Companion.voices = window.speechSynthesis.getVoices(); const select = $('companion-voice'); if (!select) return; const current = Companion.voiceName; select.innerHTML = '<option value="">الصوت المناسب تلقائيًا</option>'; Companion.voices.forEach(voice => { const option = document.createElement('option'); option.value = voice.name; option.textContent = `${voice.name} · ${voice.lang}`; select.appendChild(option); }); select.value = current; }
function stopLipSync() { const model = $('robot-model'); model?.classList.remove('robot-speaking'); Companion.speaking = false; if (Companion.lipTimer) { clearInterval(Companion.lipTimer); Companion.lipTimer = null; } }
function startLipSync() { const model = $('robot-model'); if (!model) return; stopLipSync(); model.classList.add('robot-speaking'); Companion.speaking = true; Companion.lipTimer = window.setInterval(() => { model.classList.toggle('robot-mouth-open'); }, 115); }
function speakCompanion(text) { if (!Companion.voice || !('speechSynthesis' in window) || !text.trim()) return; stopLipSync(); window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); const selected = findCompanionVoice(); if (selected) { utterance.voice = selected; utterance.lang = selected.lang; } else utterance.lang = Companion.locale; utterance.rate = Companion.rate; utterance.pitch = Companion.pitch; utterance.onstart = startLipSync; utterance.onend = stopLipSync; utterance.onerror = stopLipSync; window.speechSynthesis.speak(utterance); }
function previewCompanionVoice() { if (!('speechSynthesis' in window)) { showToast('⚠️ النطق غير مدعوم في هذا المتصفح', 'warning'); return; } const utterance = new SpeechSynthesisUtterance(Companion.locale === 'ar-MA' ? 'سلام، أنا Sentinel. نقدر نهضر معاك بالدارجة المغربية.' : Companion.locale === 'en-US' ? 'Hello, I am Sentinel. I can speak with you naturally.' : 'مرحبًا، أنا Sentinel. يمكنني التحدث معك طبيعيًا.'); const selected = findCompanionVoice(); if (selected) { utterance.voice = selected; utterance.lang = selected.lang; } utterance.rate = Companion.rate; utterance.pitch = Companion.pitch; stopLipSync(); window.speechSynthesis.cancel(); utterance.onstart = startLipSync; utterance.onend = stopLipSync; utterance.onerror = stopLipSync; window.speechSynthesis.speak(utterance); }
...
async function sendCompanionMessage() {
  ...
  try {
    const response = await fetch(`${CONFIG.API_BASE}/api/chat/stream`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: Companion.messages, locale: Companion.locale, context: getFreshWeatherContext() }), signal: Companion.controller.signal });
...
function stopCompanionStream() { Companion.controller?.abort(); Companion.controller = null; stopLipSync(); if ('speechSynthesis' in window) window.speechSynthesis.cancel(); const stop = $('companion-stop'); const send = $('companion-send'); if (stop) stop.disabled = true; if (send) send.disabled = false; }

function getFreshWeatherContext() { if (!Companion.weather) return null; if (Date.now() - Companion.weather.fetchedAt >= 1800000) { $('weather-summary').textContent = 'السياق منتهي؛ حدّث الطقس'; $('sun-times').textContent = 'الشروق — · الغروب —'; return null; } return Companion.weather; }
function weatherAdvice(code, temperature) { if (temperature >= 34) return 'الجو سخون؛ خذ الماء وقلّل من التعرض للشمس.'; if ([61,63,65,80,81,82].includes(code)) return 'كاين احتمال ديال الشتا؛ خذ احتياطك قبل ما تخرج.'; if (code === 0) return 'الجو صافي؛ فرصة زوينة للتهوية أو خرجة قصيرة.'; return 'خلي عينك على تبدّل الجو وخذ احتياطك المعتاد.'; }
function formatLocalTime(value) { return value ? new Date(value).toLocaleTimeString(Companion.locale, { hour: '2-digit', minute: '2-digit' }) : '—'; }
async function loadWeatherContext() { const summary = $('weather-summary'); const sun = $('sun-times'); const button = $('weather-location'); if (!navigator.geolocation) { summary.textContent = 'الموقع غير مدعوم'; return; } summary.textContent = 'كنطلب إذن الموقع...'; button.disabled = true; navigator.geolocation.getCurrentPosition(async position => { try { const { latitude, longitude } = position.coords; const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude.toFixed(3)}&longitude=${longitude.toFixed(3)}&current=temperature_2m,weather_code,wind_speed_10m&daily=sunrise,sunset&timezone=auto`; const response = await fetch(url); if (!response.ok) throw new Error('weather unavailable'); const data = await response.json(); const current = data.current || {}; const daily = data.daily || {}; const temperature = Math.round(current.temperature_2m); Companion.weather = { temperature, weatherCode: current.weather_code, wind: Math.round(current.wind_speed_10m || 0), sunrise: daily.sunrise?.[0], sunset: daily.sunset?.[0], advice: weatherAdvice(current.weather_code, temperature), fetchedAt: Date.now() }; summary.textContent = `${temperature}°C · رياح ${Companion.weather.wind} كم/س`; const mapPanel = $('map-panel'); const map = $('location-map'); const mapStatus = $('map-status'); const external = $('map-external-link'); const mapUrl = `https://www.google.com/maps?q=${latitude},${longitude}&output=embed`; if (map) { let mapLoaded = false; mapStatus && (mapStatus.textContent = 'جاري تحميل الخريطة بإذن الجلسة...'); map.onload = () => { mapLoaded = true; if (mapStatus) mapStatus.textContent = 'تُعرض الخريطة مؤقتًا في الجلسة ولا تُحفظ الإحداثيات.'; }; map.onerror = () => { if (mapStatus) mapStatus.textContent = 'تعذر تحميل الخريطة؛ يمكنك فتح الرابط الخارجي بدلًا منها.'; }; map.src = mapUrl; window.setTimeout(() => { if (!mapLoaded && mapStatus) mapStatus.textContent = 'الخريطة بطيئة أو غير متاحة؛ جرّب فتح الرابط الخارجي.'; }, 8000); } if (external) external.href = `https://www.google.com/maps?q=${latitude},${longitude}`; mapPanel?.classList.remove('hidden'); sun.textContent = `الشروق ${formatLocalTime(Companion.weather.sunrise)} · الغروب ${formatLocalTime(Companion.weather.sunset)}`; button.textContent = 'تم تحديث السياق'; showToast('☀️ تحدّث سياق الطقس دون حفظ موقعك', 'success'); } catch { summary.textContent = 'تعذر جلب الطقس'; sun.textContent = 'حاول مرة أخرى لاحقًا'; $('map-panel')?.classList.add('hidden'); showToast('⚠️ تعذر الوصول إلى خدمة الطقس', 'warning'); } finally { button.disabled = false; } }, () => { summary.textContent = 'تم رفض إذن الموقع'; sun.textContent = 'الشروق — · الغروب —'; button.disabled = false; showToast('⚠️ لم يتم منح إذن الموقع', 'warning'); }, { enableHighAccuracy: false, timeout: 10000, maximumAge: 300000 }); }
...
function initCompanion() { const input = $('companion-input'); const send = $('companion-send'); const stop = $('companion-stop'); const voice = $('companion-voice-toggle'); const locale = $('companion-locale'); send?.addEventListener('click', sendCompanionMessage); stop?.addEventListener('click', stopCompanionStream); voice?.addEventListener('click', () => { Companion.voice = !Companion.voice; voice.setAttribute('aria-pressed', String(Companion.voice)); voice.textContent = Companion.voice ? 'الصوت مفعّل' : 'الصوت متوقف'; if (!Companion.voice) { stopLipSync(); if ('speechSynthesis' in window) window.speechSynthesis.cancel(); } }); locale?.addEventListener('change', () => { Companion.locale = locale.value; loadCompanionVoices(); if (Companion.recognition) { stopCompanionListening(); startCompanionListening(); } }); $('companion-voice')?.addEventListener('change', event => { Companion.voiceName = event.target.value; }); $('companion-rate')?.addEventListener('input', event => { Companion.rate = Number(event.target.value); }); $('companion-pitch')?.addEventListener('input', event => { Companion.pitch = Number(event.target.value); }); $('companion-preview-voice')?.addEventListener('click', previewCompanionVoice); $('companion-listen')?.addEventListener('click', startCompanionListening); $('companion-sensors')?.addEventListener('click', enableCompanionSensors); $('weather-location')?.addEventListener('click', loadWeatherContext); $('joke-load')?.addEventListener('click', loadScientificJoke); $('joke-category')?.addEventListener('change', loadScientificJoke); loadCompanionVoices(); if ('speechSynthesis' in window) window.speechSynthesis.addEventListener('voiceschanged', loadCompanionVoices); input?.addEventListener('keydown', event => { if (event.key === 'Enter') { event.preventDefault(); sendCompanionMessage(); } }); }
...
function initRobotInterface() {
  ...
  const locale = { ar: { title: 'روبوتك الذكي، حاضر لفهم كل فكرة', description: 'رفيق صوتي مستقبلي يفهم العربية والدارجة المغربية والإنجليزية، يتفاعل معك بصريًا، ويجعل الأفعال الخارجية مشروطة بتأكيدك.', ready: 'النواة جاهزة — بانتظار أمرك', toggle: 'English', label: 'تفاعل مع الروبوت ثلاثي الأبعاد' }, en: { title: 'Your intelligent robot, ready to understand every idea', description: 'A futuristic voice companion for Arabic, Moroccan Darija, and English. It responds visually while external actions stay behind your confirmation.', ready: 'Core ready — awaiting your command', toggle: 'العربية', label: 'Interact with the 3D robot' } };
